...
any letter and then separately counting those with exactly three of any letter. 
...
def box_analyzer(boxid):
    """Count the number of times each character in boxid
        occurs; return 1 if a character occurs twice, 0 if it doesn't.
        Same for characters occurring three times
    """
    all_char_counts = {}
    for c in boxid:
        if c not in all_char_counts:
            all_char_counts[c] = 1
        else:
            all_char_counts[c] += 1
    twice = 2 in all_char_counts.values()
    thrice = 3 in all_char_counts.values()
    return (twice, thrice)


def analyze_all_boxes(fname):
    ...
    for b in boxes:
        ...
        twee += t
        drie += d
    return (twee, drie, twee * drie)


if __name__ == '__main__':
    print(analyze_all_boxes('data/day2-input.txt'))
